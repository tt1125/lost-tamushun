...
export default function Search() {
  ...
  const handleSearch = () => {
    // 固定値で画像のURLと説明を設定
    const raw_img_url = "https://picsum.photos/id/237/300/200";
    const gen_img_url = "https://picsum.photos/id/238/300/200";
    const desc = "This is a sample description.";

    setImages([raw_img_url, gen_img_url]);
    setDescription(desc);
  };
          ...
          <Grid container spacing={2} justifyContent="center" alignItems="center">
            {images.map((url, index) => (
              <Grid item xs={6} key={index}>
                <img src={url} alt={`Image ${index + 1}`} style={{ width: "100%", height: "auto", borderRadius: "8px" }} />
              </Grid>
            ))}
            <Grid item xs={12}>
